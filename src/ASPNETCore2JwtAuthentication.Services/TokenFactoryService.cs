    ...
    IDeviceDetectionService deviceDetectionService) : ITokenFactoryService
{
    ...
    public string? GetRefreshTokenSerial(string refreshTokenValue)
    {
        ...
        {
            _logger.LogError(ex, message: "Failed to validate refreshTokenValue: `{RefreshTokenValue}`.",
                refreshTokenValue);
        }
    ...
    private (string RefreshTokenValue, string RefreshTokenSerial) CreateRefreshToken()
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(ClaimTypes.System, _deviceDetectionService.GetCurrentRequestDeviceDetailsHash(), ClaimValueTypes.String,
                _configuration.Value.Issuer)
        };
    ...
    private async Task<(string AccessToken, IEnumerable<Claim> Claims)> CreateAccessTokenAsync(User user)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(type: "DisplayName", user.DisplayName ?? "", ClaimValueTypes.String, _configuration.Value.Issuer),
